...
const HighestRiskNeos = ({ selectedDate = null, limit = 10 }) => {
      ...
      <div className="w-full p-4">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-red-800">Error loading highest risk data</h3>
  ...
  if (!riskData || !riskData.highest_risk_neos || riskData.highest_risk_neos.length === 0) {
    return (
      <div className="w-full p-4">
        <div className="bg-gray-50 rounded-lg p-6 text-center">
          <p className="text-gray-600">No highest risk NEO data available for {currentDate}</p>
          <button 
            onClick={fetchHighestRiskNeos}
      ...
      <div className="bg-white rounded-lg shadow border border-blue-200 overflow-hidden">
        ...
        <div className="px-4 py-2 bg-gray-50 border-b border-blue-200">
          <p className="text-xs text-gray-600">
            Showing {displayNeos.length} of {highest_risk_neos.length} highest risk asteroids
          </p>
        </div>
        ...
        <div className="p-4">
          <div className="space-y-3">
            {displayNeos.map((neo, index) => (
              <div 
                key={neo.id}
                className={`p-3 rounded border ${getRiskBgColor(neo.risk_level)}`}
              >
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-medium text-gray-500">#{index + 1}</span>
                  <h3 className="font-medium text-gray-900 text-sm truncate flex-1">{neo.name}</h3>
                  <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${getRiskColorClass(neo.risk_level)}`}>
                    {neo.risk_level}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <div className="bg-white p-2 rounded border border-blue-200">
                    ...
                    <p className="text-gray-900 text-sm font-bold">{neo.risk_score}/100</p>
                  </div>
                  <div className="bg-white p-2 rounded border border-blue-200">
                    <h4 className="text-blue-500 text-xs font-medium">Size (Diameter)</h4>
                    <p className="text-gray-900 text-sm font-bold">{neo.size_km.toFixed(2)} km</p>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-white p-2 rounded border border-blue-200">
                    <h4 className="text-blue-500 text-xs font-medium">Distance (Lunar Distance)</h4>
                    <p className="text-gray-900 text-sm font-bold">{neo.miss_distance_lunar} LD</p>
                  </div>
                  <div className="bg-white p-2 rounded border border-blue-200">
                    <h4 className="text-blue-500 text-xs font-medium">Velocity</h4>
                    <p className="text-gray-900 text-sm font-bold">{(neo.velocity_kmh / 1000).toFixed(0)}k km/h</p>
                  </div>
                </div>
          ...
          {hasMoreThanFive && (
            <div className="mt-4 text-center">
              ...
              >
                {showAll ? `Show Less` : `Show More (${highest_risk_neos.length - 5} more)`}
              </button>
            </div>
          )}
        </div>
