  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        return res.status(404).json({ error: 'Near Earth Object not found' })
      }
      throw new Error(`NASA API error: ${response.status}`)
    }
...
app.get('/neo/closest', async (req, res) => {
  try {
    ...
    const sortedByDistance = allNeos
      .map(neo => ({
        ...neo,
        closest_distance_km: parseFloat(neo.close_approach_data[0]?.miss_distance?.kilometers || 0)
      }))
...
app.get('/neo/largest', async (req, res) => {
  try {
    ...
    const sortedBySize = allNeos
      .map(neo => ({
        ...neo,
        max_diameter_km: neo.estimated_diameter?.kilometers?.estimated_diameter_max || 0
      }))
...
app.get('/neo/summary', async (req, res) => {
  try {
    ...
    const summary = {
      ...
      largest_diameter_km: Math.max(...allNeos.map(neo => neo.estimated_diameter?.kilometers?.estimated_diameter_max || 0)),
      smallest_diameter_km: Math.min(...allNeos.map(neo => neo.estimated_diameter?.kilometers?.estimated_diameter_min || Infinity)),
      closest_approach_km: Math.min(...allNeos.map(neo => parseFloat(neo.close_approach_data[0]?.miss_distance?.kilometers || Infinity))),
      fastest_velocity_kmh: Math.max(...allNeos.map(neo => parseFloat(neo.close_approach_data[0]?.relative_velocity?.kilometers_per_hour || 0))),
      average_diameter_km: allNeos.reduce((sum, neo) => sum + (neo.estimated_diameter?.kilometers?.estimated_diameter_max || 0), 0) / allNeos.length
    }
...
app.get('/neo/charts/size-distribution', async (req, res) => {
  try {
    ...
    allNeos.forEach(neo => {
      const maxDiameter = neo.estimated_diameter?.kilometers?.estimated_diameter_max || 0
    ...
    const chartData = Object.entries(sizeCategories).map(([category, count]) => ({
      category,
      count,
    ...
    res.status(500).json({ error: 'Failed to generate size distribution data' })
  }
})
...
app.get('/neo/charts/distance-size', async (req, res) => {
  try {
    ...
    const scatterData = allNeos.map(neo => ({
      name: neo.name,
      id: neo.id,
      x: parseFloat(neo.close_approach_data[0]?.miss_distance?.kilometers || 0),
      ...
      try {
        ...
        if (response.ok) {
          ...
          timelineData.push({
            date: dateStr,
            total_count: allNeos.length,
            hazardous_count: allNeos.filter(neo => neo.is_potentially_hazardous_asteroid).length,
            closest_distance: Math.min(...allNeos.map(neo => parseFloat(neo.close_approach_data[0]?.miss_distance?.kilometers || Infinity))),
            largest_diameter: Math.max(...allNeos.map(neo => neo.estimated_diameter?.kilometers?.estimated_diameter_max || 0))
          })
        }
      } catch (error) {
...
function calculateRiskScore(neo) {
  ...
  const diameterKm = neo.estimated_diameter?.kilometers?.estimated_diameter_max || 0;
  if (diameterKm > 10) {
    riskScore += 40;
  ...
  const missDistanceKm = parseFloat(neo.close_approach_data[0]?.miss_distance?.kilometers || Infinity);
  ...
  const velocityKmh = parseFloat(neo.close_approach_data[0]?.relative_velocity?.kilometers_per_hour || 0);
...
app.get('/neo/risk-assessment', async (req, res) => {
  try {
    ...
    const riskAssessments = allNeos.map(neo => ({
      id: neo.id,
      name: neo.name,
    ...
    const riskSummary = {
      ...
      critical_risk: riskAssessments.filter(neo => neo.risk_level === 'CRITICAL').length,
      high_risk: riskAssessments.filter(neo => neo.risk_level === 'HIGH').length,
      moderate_risk: riskAssessments.filter(neo => neo.risk_level === 'MODERATE').length,
      low_risk: riskAssessments.filter(neo => neo.risk_level === 'LOW').length,
      minimal_risk: riskAssessments.filter(neo => neo.risk_level === 'MINIMAL').length,
      highest_risk_score: riskAssessments[0]?.risk_score || 0,
      average_risk_score: (riskAssessments.reduce((sum, neo) => sum + neo.risk_score, 0) / riskAssessments.length).toFixed(1)
    }
...
app.get('/neo/simple', async (req, res) => {
  try {
    ...
    const simplifiedNeos = allNeos.map(neo => ({
      id: neo.id,
      name: neo.name,
      diameter_km: neo.estimated_diameter?.kilometers?.estimated_diameter_max?.toFixed(3) || 'Unknown',
      distance_km: parseFloat(neo.close_approach_data[0]?.miss_distance?.kilometers || 0).toLocaleString(),
      velocity_kmh: parseFloat(neo.close_approach_data[0]?.relative_velocity?.kilometers_per_hour || 0).toLocaleString(),
      is_hazardous: neo.is_potentially_hazardous_asteroid,
      approach_date: neo.close_approach_data[0]?.close_approach_date || date,
      nasa_url: neo.nasa_jpl_url
    }))
